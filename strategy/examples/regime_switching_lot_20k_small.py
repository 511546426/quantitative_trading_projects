#!/usr/bin/env python3
"""
**整手真实回测 - 小本金优化版**（默认终端只输出这一条）：指定本金、100 股一手、买卖分别计费（含最低 5 元）、
按 `regime_switching_strategy_small_cap.py` 同一套信号/动态杠杆/模型止损日历做增量调仓，日收益来自 **现金+持仓权益**，
不是「理想日收益 × 本金」的数学缩放。

小数权重、无限拆分的理论曲线请直接运行 `regime_switching_strategy_small_cap.py`；本脚本加 `--show-model`
时仅多打一份模型百分比绩效作参考（仍非你的整手账户）。

运行:
  python3 strategy/examples/regime_switching_lot_20k_small.py [--capital 100000]
  python3 strategy/examples/regime_switching_lot_20k_small.py --show-model
...
_rs_path = Path(__file__).resolve().parent / "regime_switching_strategy_small_cap.py"
_spec = importlib.util.spec_from_file_location("regime_switching_strategy_small_cap", _rs_path)
...
def rebalance_delta(
    ...
    traded_notional = 0.0
    for code in sorted([c for c in deltas if deltas.get(c, 0) < 0], key=lambda c: deltas.get(c, 0)):
        sell_sh = -deltas[code]
        p = prices.get(code)
    ...
    # Defensive: Ensure codes exist in both deltas and prices before processing
    buy_codes = [
        c for c in deltas 
        if c in deltas and deltas[c] > 0 and c in prices and 
        prices[c] is not None and 
        not (isinstance(prices[c], float) and np.isnan(prices[c])) and 
        float(prices[c]) > 0
    ]
    for code in sorted(buy_codes, key=lambda c: -deltas[c]):
        buy_sh = deltas[code]
        p = float(prices[code])
        gross = buy_sh * p
        traded_notional += gross
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="整手真实回测（小本金优化版）；与 regime_switching_strategy_small_cap 同策略参数。",
    )
    ap.add_argument(
        "--capital",
        type=float,
    ...
    logger.info("=" * 60)
    logger.info("整手真实回测（小本金优化）本金=%.0f 元  show_model=%s", initial_cash, args.show_model)
    logger.info("=" * 60)
    ...
    try:
        ...
        inertia_lot = 0.30 if top_n_lot <= 14 else float(rs.INERTIA)
        logger.info(
        ...
        print("\n" + "=" * 60)
        print("  【整手真实回测（小本金优化）】100 股/手 · 最低佣 · 与主策略同权重与止损日历")
        print("=" * 60)
        ...
        if args.show_model:
            ...
            print("\n" + "=" * 60)
            print(f"  【可选参考】小数权重 · TOP_N={rs.TOP_N}（与 regime_switching_strategy_small_cap 主脚本一致）")
            print("=" * 60)
