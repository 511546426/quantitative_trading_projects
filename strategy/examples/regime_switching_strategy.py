...
因子权重（合计=1.00）:
  MA60    0.35  均值回复反转（主因子）
  RSI     0.08  超卖
  RET60   0.08  60日反转
  PB      0.16  低估值
  SIZE    0.08  小市值倾斜（降权，不再硬性排除大盘股）
  EP      0.10  盈利收益率
  MOM120  0.15  120日正向动量（买复苏趋势股）
"""
import gc
import sys
import logging
...
# ── 因子权重（总和=1.0） ──
W_MA60   = 0.25   # 均值回复（降权，避免主动惩罚大盘牛市强势股）
W_RSI    = 0.07   # RSI 超卖
W_RET60  = 0.07   # 60日反转
W_PB     = 0.16   # P/B 低估值
W_SIZE   = 0.08   # 小市值倾斜
W_EP     = 0.12   # 盈利收益率 = 1/PE
W_MOM120 = 0.25   # 120日正向动量（升权，捕捉大盘趋势年 2017/2020/2024）
...
LEVERAGE   = 2.00           # 2.0x 杠杆：目标年化20%；止损严格保护下行
...
def apply_portfolio_stop(
    ...
    min_cash_days: int = 10,       # 最短空仓天数（双均线条件已经足够保守）
    max_cash_days: int = STOP_COOLDOWN,
) -> pd.Series:
    """
    智能止损保护：回撤超 stop_loss → 清仓。
    重新入场条件（满足任一即可）:
      - 已清仓 max_cash_days 天（强制重入）
      - 已清仓至少 min_cash_days 天 且 CSI300 > 60日均线（更保守，避免熊市假信号）

    60 日均线重入的优势:
      - 2018/2011 类全年熊市: 均线持续压制 → 长期保持空仓 → 避免反复止损循环
      - 2024/09 类暴涨: 大涨足以突破 60 日均线 → 及时重入不错过
    """
    # 预计算市场条件（在 net_ret 的时间轴上对齐，避免 FutureWarning）
    # 双均线确认重入：CSI300 > 20日均线 AND CSI300 > 60日均线
    # 优势：过滤熊市假反弹（2018），同时不错过大级别复苏（2020/2024）
    if index_close is not None:
        ic = index_close.reindex(net_ret.index).ffill()
        ma20_r = ic.rolling(20, min_periods=10).mean()
        ma60_r = ic.rolling(60, min_periods=30).mean()
        above_ma20 = ic.shift(1) > ma20_r.shift(1)
        above_ma60 = ic.shift(1) > ma60_r.shift(1)
        market_ok  = (above_ma20 & above_ma60).fillna(True)
    else:
        market_ok = pd.Series(True, index=net_ret.index)
    ...
    for i in range(len(net_ret)):
        if in_cash:
            ...
            market_reenter = (days_in_cash >= min_cash_days) and bool(market_ok.iloc[i])
            if force_reenter or market_reenter:
                in_cash = False
                days_in_cash = 0
                peak = nav          # 重置高点，避免立即再触发
        else:
    ...
    logger.info(
        "止损保护: 清仓 %d 天 (占 %.1f%%)，止损=%.0f%%，最短等待=%d天，最长=%d天，重入基准=60日均线",
        total_cash_days,
...
def main():
    ...
    logger.info("组合: TOP=%d  调仓=%dd  惯性=%.2f  杠杆=%.1fx  止损=%.0f%%", TOP_N, REBAL_FREQ, INERTIA, LEVERAGE, STOP_LOSS * 100)
    logger.info("=" * 60)
