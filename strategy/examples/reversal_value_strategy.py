...
REBAL_FREQ   = 42           # 调仓周期（交易日），约 2 个月
W_MA60       = 0.50         # MA60 偏离率权重（最强反转因子）
W_RSI        = 0.15         # RSI 超卖权重
W_RET20      = 0.15         # 20 日反转权重
W_PB         = 0.20         # P/B 价值因子权重（低估值+反转复合）
VOL_CUTOFF   = 0.7          # 波动率过滤：剔除排名最高的 30%
INERTIA      = 0.10         # 持仓惯性加分
LEVERAGE     = 0.80         # 基础杠杆（经验最优：信号净 alpha 约 4.5%/年）
...
# 择时参数：用指数趋势做“降仓”，降低回撤
TIMING_ON           = False  # 关闭：截面反转策略择时持续无效
TIMING_USE_INDEX    = True   # 使用沪深300指数MA择时
...
TIMING_LEVELS = [
    (0.02,  1.00),   # 中位价MA20 > MA80 2%：bull，满仓
    (0.00,  0.90),   # 中位价MA20 > MA80：中性，90%仓位
    (-0.03, 0.45),   # 中位价MA20 < MA80 3%：熊市信号，降至45%
    (-0.07, 0.15),   # 中位价MA20 < MA80 7%：深熊，保留15%底仓
    (-1.00, 0.05),   # 极端深熊，几乎空仓
]
...
def load_price(ch) -> tuple[pd.DataFrame, pd.DataFrame]:
    """
    按年分块加载复权收盘价和成交额（float32）。
    分块策略避免将全量 16M 行 Python 对象同时驻留内存（否则峰值 >1.5GB）。
    """
    start_year = int(START[:4])
    end_year   = int(END[:4])

    chunks_close:  list[pd.DataFrame] = []
    chunks_amount: list[pd.DataFrame] = []
    ...
    for year in range(start_year, end_year + 1):
        ...
        close_y  = df.pivot(index="trade_date", columns="ts_code", values="adj_close")
        amount_y = df.pivot(index="trade_date", columns="ts_code", values="amount")
        del df; gc.collect()
        chunks_close.append(close_y)
        chunks_amount.append(amount_y)
        del close_y, amount_y; gc.collect()
        logger.info("  已加载 %d 年", year)

    close  = pd.concat(chunks_close,  axis=0, sort=True).sort_index()
    del chunks_close;  gc.collect()
    amount = pd.concat(chunks_amount, axis=0, sort=True).sort_index()
    del chunks_amount; gc.collect()

    logger.info("行情数据: %d 交易日 × %d 只股票", *close.shape)
    return close, amount


def load_valuation(pg) -> tuple[pd.DataFrame, pd.DataFrame]:
    """
    按年分块加载 P/B 和流通市值 circ_mv（万元）估值数据。
    Returns: (pb_df, circ_mv_df) — 均为 (trade_date × ts_code) float32 矩阵
    circ_mv 单位：万元（Tushare 标准），30亿 = 300,000 万元
    """
    start_year = int(START[:4])
    end_year   = int(END[:4])

    chunks_pb:  list[pd.DataFrame] = []
    chunks_mv:  list[pd.DataFrame] = []

    for year in range(start_year, end_year + 1):
        y_start = f"{year}-01-01"
        y_end   = f"{year}-12-31"
        rows = pg.execute_query(
            "SELECT ts_code, trade_date, pb, circ_mv FROM daily_valuation "
            "WHERE trade_date >= %s AND trade_date <= %s",
            (y_start, y_end),
        )
        if not rows:
            continue
        df = pd.DataFrame(rows, columns=["ts_code", "trade_date", "pb", "circ_mv"])
        df["trade_date"] = pd.to_datetime(df["trade_date"])
        df[["pb", "circ_mv"]] = df[["pb", "circ_mv"]].astype(np.float32)
        pb_y = df.pivot(index="trade_date", columns="ts_code", values="pb")
        mv_y = df.pivot(index="trade_date", columns="ts_code", values="circ_mv")
        del df; gc.collect()
        chunks_pb.append(pb_y)
        chunks_mv.append(mv_y)
        del pb_y, mv_y; gc.collect()

    pb = pd.concat(chunks_pb, axis=0, sort=True).sort_index()
    del chunks_pb; gc.collect()
    mv = pd.concat(chunks_mv, axis=0, sort=True).sort_index()
    del chunks_mv; gc.collect()

    logger.info("P/B + 市值数据: %d 交易日 × %d 只股票", *pb.shape)
    return pb, mv


def load_roe(pg, date_index: pd.DatetimeIndex, all_codes: pd.Index) -> pd.DataFrame | None:
    """
    加载最近一期披露的 ROE（净资产收益率），用于质量因子。
    financial_indicator 为季报/年报，使用 ann_date（披露日期）做"点位时间"前向填充，
    避免未来信息泄露。
    Returns: DataFrame (trade_date × ts_code) float32，缺失值为 NaN
    """
    try:
        rows = pg.execute_query(
            "SELECT ts_code, ann_date, roe FROM financial_indicator "
            "WHERE ann_date >= %s AND ann_date <= %s AND roe IS NOT NULL",
            (str(date_index.min().date()), str(date_index.max().date())),
        )
    except Exception as e:
        logger.warning("ROE 数据加载失败: %s", e)
        return None

    if not rows:
        return None

    df = pd.DataFrame(rows, columns=["ts_code", "ann_date", "roe"])
    df["ann_date"] = pd.to_datetime(df["ann_date"])
    df["roe"] = df["roe"].astype(np.float32)

    # 保留每支股票每个披露日最新记录（同日多条取均值）
    df = df.groupby(["ts_code", "ann_date"])["roe"].mean().reset_index()

    # 以 ann_date 为 index 展宽为矩阵，然后 reindex 到交易日并前向填充
    roe_wide = df.pivot(index="ann_date", columns="ts_code", values="roe")
    roe_aligned = (
        roe_wide
        .reindex(index=date_index.union(roe_wide.index))
        .sort_index()
        .ffill()
        .reindex(index=date_index)
        .reindex(columns=all_codes)
        .astype(np.float32)
    )
    logger.info("ROE 数据: %d 交易日 × %d 只股票", *roe_aligned.shape)
    return roe_aligned
...
# 股票池构建
# ─────────────────────────────────────────────────────────────
SMALL_CAP_MAX_MV = 5_000_000   # 小中市值上限：500亿（宽松保留，避免压缩股票池）
...
def build_universe(
    ...
    可交易股票池：
      ...
      - 排除极端下跌股：价格低于 52 周最高价 35% 以下的股票
      - [新] 专注小中市值（流通市值 < 30亿）：A 股反转效应在此段最强
    """
    mask = close.notna()
    ...
    del avg_amount; gc.collect()

    # 52 周高点过滤：剔除价格低于 52 周最高价 35% 以下的股票
    high_52w = close.rolling(252, min_periods=60).max()
    not_fallen_knife = (close / high_52w) >= 0.35
    del high_52w; gc.collect()
    mask = mask & not_fallen_knife
    del not_fallen_knife; gc.collect()

    # 小中市值过滤：只保留流通市值 < SMALL_CAP_MAX_MV 的股票
    # NaN 填 True = 无数据则不排除（避免早期数据稀缺把整个股票池清空）
    if circ_mv is not None:
        mv_aligned = circ_mv.reindex(index=close.index, columns=close.columns).ffill()
        small_cap_mask = (mv_aligned > 0) & (mv_aligned < SMALL_CAP_MAX_MV)
        mask = mask & small_cap_mask.fillna(True)   # NaN = 保留，不排除
        del mv_aligned, small_cap_mask; gc.collect()
...
def calc_factors(
    ...
    del gain, loss; gc.collect()
    r_rsi = (-rsi).where(universe).rank(axis=1, pct=True)
    del rsi; gc.collect()

    # 因子 3: 20 日收益率反转
    ret20   = close / close.shift(20) - 1
    r_ret20 = (-ret20).where(universe).rank(axis=1, pct=True)
    del ret20; gc.collect()

    # 因子 4: P/B 价值因子（低估值 = 高信号，若估值数据可用）
    if pb is not None:
        pb_aligned = pb.reindex(index=close.index, columns=close.columns).ffill()
        pb_pos = pb_aligned.where(pb_aligned > 0.1)
        r_pb = (-pb_pos).where(universe).rank(axis=1, pct=True)
        del pb_aligned, pb_pos; gc.collect()
    else:
        r_pb = None
    ...
    del vol_mask, qual_mask; gc.collect()

    # 加权合成信号
    has_pb = r_pb is not None
    if has_pb:
        signal = (W_MA60 * r_ma60 + W_RSI * r_rsi + W_RET20 * r_ret20 + W_PB * r_pb).where(tradeable)
        del r_pb
    else:
        w_sum = W_MA60 + W_RSI + W_RET20
        signal = ((W_MA60/w_sum) * r_ma60 + (W_RSI/w_sum) * r_rsi + (W_RET20/w_sum) * r_ret20).where(tradeable)
    del r_ma60, r_rsi, r_ret20, tradeable; gc.collect()

    logger.info(
        "因子权重: MA60=%.2f  RSI=%.2f  Ret20=%.2f  PB=%.2f  vol_cutoff=%.1f",
        W_MA60, W_RSI, W_RET20, W_PB if has_pb else 0.0, VOL_CUTOFF,
    )
    return signal
...
def apply_drawdown_overlay(
    ...
    """
    基于策略自身回撤的动态降仓：
      - 利用"无覆盖"组合的历史净值回撤来确定当日杠杆系数（T-1 滞后，无前瞻偏差）
      - leverage = clip(1 - abs(dd) * dd_scale, min_leverage, 1.0)
        回撤 -10%: leverage ≈ 0.70  |  -20%: ≈ 0.40  |  -33%: ≈ 0.08
    """
    daily_ret = close.pct_change(fill_method=None).fillna(0).clip(-0.2, 0.2)
    base_ret  = (weights.shift(1) * daily_ret).sum(axis=1)
    nav       = (1 + base_ret).cumprod()
    dd        = nav / nav.cummax() - 1           # 当日回撤（≤0）

    leverage = (1.0 + dd * dd_scale).clip(lower=min_leverage, upper=1.0)
    leverage = leverage.shift(1).fillna(1.0)     # T-1 对齐，避免前瞻

    adjusted = weights.mul(leverage, axis=0)
...
def main():
    ...
    logger.info("持仓: %d 只  调仓: 每 %d 日  因子: MA60(%.2f)+RSI(%.2f)+Ret20(%.2f)+PB(%.2f)",
                TOP_N, REBAL_FREQ, W_MA60, W_RSI, W_RET20, W_PB)
    logger.info(
    ...
    # 估值数据（P/B + 市值）：年度分块加载，内存可控
    logger.info("加载 P/B + 市值估值数据...")
    try:
        pb_data, mv_data = load_valuation(pg)
    except Exception as e:
        logger.warning("估值数据加载失败，回退到无价值因子/市值过滤模式: %s", e)
        pb_data, mv_data = None, None

    universe = build_universe(close, amount, exclude, circ_mv=mv_data)
    del amount, mv_data; gc.collect()

    # 裁剪矩阵：剔除整段历史中从未进入股票池的列，大幅降低后续峰值内存
    active_cols = universe.any(axis=0)
    if active_cols.sum() < universe.shape[1]:
        n_before = universe.shape[1]
        close    = close.loc[:, active_cols]
        universe = universe.loc[:, active_cols]
        logger.info("裁剪矩阵: %d → %d 只（删除从未入池列）", n_before, active_cols.sum())
    del active_cols; gc.collect()
    ...
    # ── 子策略信号与权重 ──
    logger.info("计算反转+价值复合信号...")
    signal_rev = calc_factors(close, universe, pb=pb_data)
    del pb_data; gc.collect()
